export default function playMethodsSyx5 (type, detial, selectedData) {
  ...
  if(type === '三码' && detial === '前三直选复式') {
    ...
    let firstNumArr = []
    for (let i in selectedData[0].mulActive) {
      if (selectedData[0].mulActive[i]) {
        firstNumArr.push(selectedData[0].numberArr[i])
      }
    }
    let secondNumArr = []
    for (let i in selectedData[1].mulActive) {
      if (selectedData[1].mulActive[i]) {
        secondNumArr.push(selectedData[1].numberArr[i])
      }
    }
    let thirdNumArr = []
    for (let i in selectedData[2].mulActive) {
      if (selectedData[2].mulActive[i]) {
        thirdNumArr.push(selectedData[2].numberArr[i])
      }
    }
    // console.log('的开发进度')
    // console.log(firstNumArr)
    // let secondNumArr = selectedData[1].mulActive.map(item => item.label)
    // let thirdNumArr = selectedData[2].mulActive.map(item => item.label)
    // 处理1，2级的
  ...
  if(type === '三码' && detial === '前三组选复式') {
    ...
    for (let i in selectedData[0].mulActive) {
      if (selectedData[0].mulActive[i]) {
        selectedNum.push(selectedData[0].numberArr[i])
      }
    }
    // 三个为一注，c53=10的关系
